// https://editor.p5js.org/jht9629-nyu/sketches/vF9SPE6kL
// CC 181: Stippling Video v1

// https://editor.p5js.org/karan.suthar.lakha/sketches/oBp4F8b8M
// CC 181: Stippling Video copy by karan.suthar.lakha
...
function draw() {
  ...
  if (started) {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      let g = video.pixels[index + 1];
      let b = video.pixels[index + 2];
      let sw = map(avgWeights[i], 0, maxWeight, 0, strokeWeightMax, true);
      strokeWeight(sw);
      point(v.x, v.y);
    }
